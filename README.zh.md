...
ClawX 提供了一套完整的 Systemd 服务来托管你的智能体。运行以下命令即可一键安装整个生命维持系统：
```bash
...
ClawX 的核心灵魂在于其动态的**心情系统**。智能体的行为不是随机的，而是由一组演化的参数驱动的：
